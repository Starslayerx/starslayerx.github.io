...
class ContextCompactionController {
  ...
  static async shouldCompact(
    ...
    return (
      tokenCount > this.COMPACTION_THRESHOLDS.tokenCount ||
...
class SystemPromptAssembler {
  ...
  static async assemble(
    ...
    // 并行获取动态组件
    const [claudeMdSection, gitSection, directorySection, toolSection] =
      await Promise.all([
...
graph TB
    subgraph "工具请求分析"
        ...
        Categorize -->|只读| ReadQueue[读取队列]
        Categorize -->|写入/副作用| WriteQueue[写入队列]
    end
...
class ErrorRecoveryController {
  ...
  private static async *handleRateLimit(
    ...
    for (const provider of alternatives) {
      ...
      try {
        const result = await this.retryWithProvider(context.request, provider);
        yield* result;
        return;
      } catch (e) {
        continue;
...
class PerformanceProfiler {
  ...
  static instrument<T extends AsyncGenerator>(name: string, generator: T): T {
